...
function start(images, fonts, defaultTexts, title, makeContext) {
	...
	btnImage.onclick = function(event) {
		...
		wnd.document.write('<style>body {cursor:wait; background: #ddd; margin: 5px;}</style><pre style="font: 14px monospace;">' + JSON.stringify(json) + '</pre><br><pre style="font: 14px monospace;">JOB IN PROGRESS...</pre>');
		wnd.document.title = document.title + ' (Job in progress...)';
		//
		var ctx = makeContext(_imgs, _bgImg, json);
...
function restoreUi(w, h, defaultTexts) {
	var defaultJson = getDefaultJson(w, h, defaultTexts);
